...
- **`brand_daily_budget_changed` is byte-equal in THREE places** and must stay so: the event key billing sends, the template `name` registered at boot in `src/instrument.ts`, and the entry in transactional-email-service's `ADMIN_NOTIFICATION_EVENTS` (its PR #108). That service resolves a template by `name == eventType`, so renaming one of the three silently 404s the send.
- **`src/instrument.ts` MUST stay imported by `src/index.ts`** (called after `app.listen()`, not awaited). It was an ORPHAN module — zero importers — from the day it was written until 2026-07-30, so billing had never registered a single template: the prod email DB held no `credits-reload-failed` row (that send had been 404-ing for months) and, on the first ship, no `brand_daily_budget_changed` row either — the feature deployed green and inert. **A module whose only effect is a side effect at import time is invisible when nothing imports it: verify template registration against the email service's DB (`SELECT name FROM email_templates WHERE name IN (…)` on the `transactional-email` Neon project), never by reading `instrument.ts` and assuming it runs.** It now uses `PUT /platform-templates` (api-key only, the cold-start route) instead of the identity-gated `PUT /templates` twin, so no fake all-zero user identity is invented; both are per-name upserts, so billing never disturbs another service's rows. `credits-depleted` was dropped from the set — nothing sends it (dunning sends the dashboard-owned `credit-depleted` family).
