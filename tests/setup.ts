...
beforeAll(async () => {
  ...
  // campaign_daily_budgets (one daily ceiling per campaign, migration 0048).
  await sql`
    CREATE TABLE IF NOT EXISTS "campaign_daily_budgets" (
      "org_id" uuid NOT NULL,
      "brand_id" uuid NOT NULL,
      ...
      "daily_budget_cents" numeric(16,10) NOT NULL,
      "updated_at" timestamp with time zone DEFAULT now() NOT NULL,
      CONSTRAINT "campaign_daily_budgets_campaign_key"
        UNIQUE NULLS NOT DISTINCT ("org_id", "brand_id", "feature_slug", "offer_id", "leg_key")
    )
  `;
  await sql`
    CREATE INDEX IF NOT EXISTS "campaign_daily_budgets_org_brand_idx"
      ON "campaign_daily_budgets" ("org_id", "brand_id")
  `;
